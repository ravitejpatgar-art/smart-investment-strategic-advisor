...
export const OnboardingWizard: React.FC = () => {
  ...
  // Restore draft from persistence on mount / user change
  useEffect(() => {
    if (hydratedForUidRef.current === effectiveUid && hasHydrated) return;
    ...
    if (draft) {
      ...
      if (draft.monthlySalary) setMonthlySalary(draft.monthlySalary);
      if (draft.otherIncome) setOtherIncome(draft.otherIncome);
      if (draft.emergencyFund) setEmergencyFund(draft.emergencyFund);
      if (draft.existingInvestments) setExistingInvestments(draft.existingInvestments);
      if (draft.savingsBalance) setSavingsBalance(draft.savingsBalance);
      if (draft.rent) setRent(draft.rent);
      if (draft.food) setFood(draft.food);
      if (draft.transport) setTransport(draft.transport);
      if (draft.emi) setEmi(draft.emi);
      if (draft.selectedGoals && draft.selectedGoals.length > 0) setSelectedGoals(draft.selectedGoals);
      if (draft.riskAnswers) setRiskAnswers(draft.riskAnswers);
      if (draft.investmentHorizon) setInvestmentHorizon(draft.investmentHorizon);
      if (typeof draft.includeGlobalAssets === 'boolean') setIncludeGlobalAssets(draft.includeGlobalAssets);
      if (typeof draft.includeGoldHedge === 'boolean') setIncludeGoldHedge(draft.includeGoldHedge);
      if (typeof draft.directPlansOnly === 'boolean') setDirectPlansOnly(draft.directPlansOnly);
    }
    hydratedForUidRef.current = effectiveUid;
    setHasHydrated(true);
  }, [effectiveUid, hasHydrated]);
  ...
  const handleFinish = () => {
    userProfileRepo.clearOnboardingDraft(effectiveUid);
